function [U,D,V, fronorm] = accuracyEnhancedRSVD2(A,k,p,q)
    [m,n] = size(A);
    G = randn(n, k+p);
    Y = A*G;
    for j = 1:q
...
disp("Average Theoretical frobrenius norm = "+norm(y_theo_fronorm));
disp("Average Observed Spectral norm = "+mean(y_spectral));
disp("Average Theoretical Spectral norm = "+norm(y_theo_fronorm));
